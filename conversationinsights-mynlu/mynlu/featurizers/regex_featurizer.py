...
class RegexFeaturizer(Featurizer):
    ...
    def train(self, training_data, config, **kwargs):
        ...
        for regex_pattern in training_data.regex_features:
            self.known_patterns.append(regex_pattern)
    ...
    def features_for_patterns(self, message):
        ...
        message: a sentence,
        return: a vector of {1,0} values indicating which regexes did match.
        Furthermore, if the message is tokenized, the function will mark the matching regex on the tokens
        ...
        for i, exp in enumerate(self.known_patterns):
            ...
            if match is not None:
                ...
                logger.debug("match.start/end: {}/{}".format(match.start(), match.end()))
                for t in message.get("tokens", []):
                    logger.debug("message.token:{}".format(t))
