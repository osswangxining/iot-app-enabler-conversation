...
from mynlu.config import MyNLUConfig, InvalidConfigError
from mynlu.registry import registered_pipeline_templates
...
def test_invalid_config_json():
    ...
    with tempfile.NamedTemporaryFile("w+", suffix="_tmp_config_file.json") as f:
        f.write(file_config)
        f.flush()
        with pytest.raises(mynlu.config.InvalidConfigError):
            MyNLUConfig(f.name, env_vars, cmdline_args)


def test_automatically_converts_to_unicode():
    env_args = {"some_test_key_u": str("some test value"), str("some_test_key_str"): str("some test value")}
    cmd_args = {"some_other_test_key_str": str("some test value"), str("some_other_test_key_u"): str("some test value")}
    final_config = MyNLUConfig(CONFIG_DEFAULTS_PATH, env_args, cmd_args)
