...
from mynlu.cli.convert import convert_training_data
from mynlu.converters import load_data, validate_mynlu_data
...
def test_example_training_data_is_valid():
    with io.open('data/examples/mynlu/demo-mynlu.json', encoding="utf-8-sig") as f:
...
@pytest.mark.parametrize("invalid_data", [
    ...
    {"mynlu_data": {"common_examples": [{"intent": "some example without text"}]}},
    {"mynlu_data": {
        "common_examples": [{
...
def test_validation_is_throwing_exceptions(invalid_data):
    with pytest.raises(ValidationError):
        validate_mynlu_data(invalid_data)
...
def test_mynlu_data():
    td = load_data('data/examples/mynlu/demo-mynlu.json')
    assert td.entity_examples != []
    assert td.intent_examples != []
...
@pytest.mark.parametrize("data_file,gold_standard_file,output_format", [
    ("data/examples/wit/demo-flights.json", "data/test/wit_converted_to_mynlu.json", "json"),
    ("data/examples/luis/demo-restaurants.json", "data/test/luis_converted_to_mynlu.json", "json"),
    ("data/examples/api/", "data/test/api_converted_to_mynlu.json", "json"),
    ("data/examples/mynlu/demo-mynlu.md", "data/test/md_converted_to_json.json", "json"),
    ("data/examples/mynlu/demo-mynlu.json", "data/test/json_converted_to_md.md", "md")
])
